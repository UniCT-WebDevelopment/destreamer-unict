...
export const Error: IError = {
    ...
    [ERROR_CODE.NO_LOGINDATA_FILE]:     'Login data file not found!',

    [ERROR_CODE.READ_LOGDATA_FAIL]:     'Could not read login data file!',
}
...
export const enum CLI_ERROR {
    ...
    MISSING_REQUIRED_ARG      = 'You must specify a URLs source.\n' +
                                'Valid options are -i or -f.',

    VIDEOURLS_ARG_CONFLICT    = 'Too many URLs sources specified!\n' +
                                'Please specify a single URLs source with either -i or -f.',

    OUTPUTDIR_ARG_CONFLICT    = 'Too many output arguments specified!\n' +
                                'Please specify a single output argument with either -o or -O.',

    FILE_INPUT_VIDEOURLS_ARG  = 'Wrong input for option -i.\n' +
                                'To read URLs from file, use -f option.',
